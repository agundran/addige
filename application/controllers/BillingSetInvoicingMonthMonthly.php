...
class BillingSetInvoicingMonthMonthly extends CI_Controller
{
	private $limit = 12;
	...
	#load library dan helper yang dibutuhkan
		...
		$this->load->library(array('table','form_validation'));
	 	...
	 	$this->load->model('BillingSetInvoicingMonthMonthly_model','',TRUE);
 	}
	...
	anchor_popup(array('BillingSetInvoicingMonthMonthly/update/'.$user1,$Users->Year,$Users->Month,$Users->Start_Date, $Users->End_Date),'Select',array('class'=>'update'),$atts));
...
function update($id,$year,$month, $sd, $ed){
	...
	// set validation properties
	 	$this->_set_rules();
	 	$data['action'] = ('BillingSetInvoicingMonthMonthly/update/'.$id."/".$year."/".$month."/".$sd."/".$ed);
	...
	// run validation
		...
		$data['title'] = 'Set Monthly Calendar';
	...
	// save data
		...
		//$month = $this->input->post('Month');
	 	...
	 	$this->BillingSetInvoicingMonthMonthly_model->update($id,$User);
	...
	function user_exists($username){
    	$this->BillingSetInvoicingMonthMonthly_model->user_exists($username);
	}
