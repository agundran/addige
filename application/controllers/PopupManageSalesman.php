...
class PopupManageSalesman extends CI_Controller
{
	private $limit = 5;
	...
	$config['base_url'] = site_url('/PopupManageSalesman/index');
	...
	anchor('PopupManageSalesman/index/'.$offset.'/Sales/'.$new_order, 'Sales'),
	anchor('PopupManageSalesman/index/'.$offset.'/City/'.$new_order, 'City'),
	anchor('PopupManageSalesman/index/'.$offset.'/State/'.$new_order, 'State'),
	...
	anchor('PopupManageSalesman/delete/'.$Users->Seq,'delete',array('class'=>'delete','onclick'=>"return confirm('Are you sure you want to remove this Operator?')"))
	);
	...
	// load view
		...
		$this->load->view('pages/PopupManageSalesman_view', $data);
	...
	$data['action'] = site_url('PopupManageSalesman/validate_add');
	$data['link_back'] = anchor('/PopupManageSalesman/index/','Back to Operators list',array('class'=>'back'));
	...
	// run validation
		if ($this->form_validation->run() === FALSE){
			...
			$data['link_back'] = anchor('PopupManageSalesman/index/','See List Of Users',array('class'=>'back'));
				...
				$this->load->view('pages/template/header');
			//	$this->load->view('pages/template/nav', $data);
				$this->load->view('pages/PopupManageSalesmanEdit_view', $data);
			//	$this->load->view('pages/template/footer');
			...
			redirect('PopupManageSalesman/index/add_success','Refresh');
	...
	// set validation properties
	 	$this->_set_rules();
	 	$data['action'] = ('PopupManageSalesman/update/'.$id);
	...
	// set user message
	$data['message'] = 'Salesman Update successful';
		//$message = "Operator Update successful";
	 	...
	 	$data['message'] = 'update Salesman Data success';
		redirect('PopupManageSalesman/index/add_success','Refresh');
	...
	// redirect to Operator list page
	 	redirect('PopupManageSalesman/index/delete_success','refresh');
