...
class pendingtesting2 extends CI_Controller
{
	//private 
	function __construct()
 	{
		parent::__construct();
	 	...
	 	$this->load->model('pendingtestingmodel','',TRUE);
 		$this->is_logged_in();
 	}
	...
	function index($offset = 0, $order_type = 'asc')
	{
		...
		// load data
		//$data['Users']= $this->pendingtestingmodel->get_paged_list($limit, $offset, $order_type, $filter);
		// generate pagination
	...
	//	$data['table'] = $this->table->generate();
		...
		$this->load->view('pages/template/headerpending');
		$this->load->view('pages/template/nav',$data);
		$this->load->view('pages/pendingtesting2view', $data);
...
for($i=0; $i < $N; $i++)
{
	$result = mysql_query("UPDATE contract_header SET Attributes = '256' where Seq='$id[$i]'");
	
}
redirect('pendingtesting2/index','');
