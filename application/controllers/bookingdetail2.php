...
class Bookingdetail2 extends CI_Controller{	
	...
	function __construct()
 	{
      ...
      //loads library and , helper, model	 
		...
		$this->load->library(array('table','form_validation'));
	 	...
	 	$this->load->model('bookingdetailmodel2','',TRUE);
		$this->is_logged_in();
 	}
	...
	$astartdate = $this->bookingdetailmodel2->get_billingstartdate($currentuser);
	$aenddate = $this->bookingdetailmodel2->get_billingenddate($currentuser);
	...
	$Users = $this->bookingdetailmodel2->get_paged_list()->result();
	 
	// generate pagination
	$this->load->library('pagination');
	$config['base_url'] = site_url('/bookingdetail2/index/');
	$config['total_rows'] = $this->bookingdetailmodel2->count_sitename_active();
	$config['per_page'] =  $this->bookingdetailmodel2->count_sitename_active();
   ...
   // generate table data
	...
	$bookedgross = $this->bookingdetailmodel2->computepersite($Users->t2SN, $astartdate, $aenddate);
	$bookednet = $this->bookingdetailmodel2->computepersitenet($Users->t2SN, $astartdate, $aenddate );
	//$bookednet = 0;
	...
	return $mytotcon;	 
	 ...
	 function view($sitename,$bookedgross){
		...
		// $data['net'] = "0.0";
		 $data['net'] = $this->bookingdetailmodel2->computepersitenet($sitename);
