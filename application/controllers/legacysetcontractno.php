...
class Legacysetcontractno extends CI_Controller
{
	private $limit = 12;
	...
	function __construct()
 	{
		...
		$this->load->helper(array('form', 'url'));
	 	$this->load->model('legacysetcontractnomodel','',TRUE);
 		$this->is_logged_in();
 	}
	...
	$data['action'] = site_url('legacysetcontractno/index/');
	

	$data['currentuser'] = $this->session->userdata('username');
	$myuser = $data['currentuser'];
	$astartdate = $this->legacysetcontractnomodel->get_billingstartdate($myuser);
	$aenddate = $this->legacysetcontractnomodel->get_billingenddate($myuser);
	$cm = $this->legacysetcontractnomodel->get_currentmonth($myuser);
   ...
   // $data['sc'] = "9129";
	...
	// load view
		...
		$this->load->view('pages/legacysetcontractno_view', $data);
	...
	function search()
{
	...
	$EndDate = date('Y-m-d',strtotime($aenddate));
		...
		//$sd = $this->legacysetcontractnomodel->get_billingstartdate($currentuser);
	//	$ed= $this->legacysetcontractnomodel->get_billingenddate($currentuser);  
	
		$data['title1'] = 'Current Billing Period is: '.$cm." ".'('.$StartDate." "."to"." ".$EndDate.")";	
		$data['setbm'] = anchor_popup('BillingSetUpdate/index/','(change)',array('class'=>'changebillingmonth'),$upd);
		
		$Users = $this->legacysetcontractnomodel->get_paged_list($this->limit, $offset, $order_column, $order_type)->result();
	
	$this->load->library('pagination');
		$config['base_url'] = site_url('/legacysetcontractno/search/');
		
		//$config['total_rows'] = $this->selectsitemodel->count_all($order_column);
		$config['total_rows'] = $this->legacysetcontractnomodel->count_all($order_column, $astartdate, $aenddate);
