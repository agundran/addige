...
class Booking2 extends CI_Controller
{
	...
	#load library dan helper yang dibutuhkan
		...
		$this->load->library(array('table','form_validation'));
	 	...
	 	$this->load->model('bookingmodel2','',TRUE);
 		$this->is_logged_in();
 	}
	...
	// load data
	$Users = $this->bookingmodel2->get_paged_list($this->limit, $offset, $order_column, $order_type, $filter)->result();
	 
	// generate pagination
	$this->load->library('pagination');
	$config['base_url'] = site_url('/booking2/index');
	$config['total_rows'] = $this->bookingmodel2->count_all();
	...
	anchor(array('bookingdetail2/index/'.$Users->Year,$Users->Month),'Select',array('class'=>'select'))
	);
