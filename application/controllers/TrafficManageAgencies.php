...
class TrafficManageAgencies extends CI_Controller
{
	private $limit = 20;
	...
	$config['base_url'] = site_url('/TrafficManageAgencies/index');
	...
	anchor('TrafficManageAgencies/index/'.$offset.'/Name/'.$new_order, 'Agency'),
	...
	anchor_popup('TrafficManageAgencies/update/'.$Users->Seq,'Update',array('class'=>'update'), $upd).'   '.
	...
	anchor('TrafficManageAgencies/delete/'.$Users->Seq,'delete',array('class'=>'delete','onclick'=>"return confirm('Are you sure you want to remove this Agency?')"
	...
	// load view
		...
		$this->load->view('pages/TrafficManageAgenciesList_view', $data);
	...
	// run validation
		if ($this->form_validation->run() === FALSE){
			...
			$message = "Agency ".$User['Name']." successfully added!";
	...
	// set user message
		...
		$message = "Agency Updated successful";
   			 if ((isset($message)) && ($message != '')) {
        		echo '<script>
           		 alert("'.str_replace(array("\r","\n"), '', $message).'");
           		 window.opener.location.reload();
