...
class Broadcastcalendar extends CI_Controller
{
	private $limit = 12;
	...
	#load library dan helper yang dibutuhkan
		...
		$this->load->library(array('table','form_validation'));
	 	...
	 	$this->load->model('broadcastcalendarmodel','',TRUE);
 		$this->is_logged_in();
 	}
	...
	anchor('broadcastcalendar/index/'.$offset.'/Year/'.$new_order, 'Year'),
	anchor('broadcastcalendar/index/'.$offset.'/Month/'.$new_order, 'Month'),
	anchor('broadcastcalendar/index/'.$offset.'/Start_Week/'.$new_order, 'Start Week'),
	anchor('broadcastcalendar/index/'.$offset.'/End_Week/'.$new_order, 'End Week'),
	anchor('broadcastcalendar/index/'.$offset.'/Start_Date/'.$new_order, 'Start Date'),
	anchor('broadcastcalendar/index/'.$offset.'/End_Date/'.$new_order, 'End Date'),
	'Actions',"");
	 $upd = array(
              'width'      => '800',
              'height'     => '600',
	...
	anchor_popup(array('broadcastcalendar/update/'.$Users->Year,$Users->Month),'Update',array('class'=>'update'), $upd),
	anchor(array('broadcastcalendar/delete/'.$Users->Year,$Users->Month),'Delete',array('class'=>'delete','onclick'=>"return confirm('Are you sure you want to remove this Calendar Entry?')"))
	);
	...
	{
	$data['title'] = 'Add Broadcast Calendar';
	$data['action'] = site_url('broadcastcalendar/validate_add');
	...
	// run validation
		if ($this->form_validation->run() === FALSE){
			...
			$data['title'] = 'Add new Broadcast Calendar';
			$data['message'] = '';
			$data['Users']['Year']='';
			$data['Users']['Month']='';
			...
			$data['Role']=$this->session->userdata('role');
				...
				$this->load->view('pages/template/header2');
				$this->load->view('pages/broadcastcalendaredit_view', $data);
			...
			$id = $this->broadcastcalendarmodel->create_calendar($year,$month,$startweek,$endweek,$startdate,$enddate);
	...
	// set validation properties
	 	$this->_set_rules();
	 	$data['action'] = ('broadcastcalendar/update/'.$year."/".$month);
	...
	// save data
		...
		$month = $this->input->post('Month');
	 	...
	 	$this->broadcastcalendarmodel->update($year,$month,$User);
	...
	function delete($year,$month){
	 // delete Operator
	 	$this->broadcastcalendarmodel->delete($year,$month);
	 
	// redirect to Operator list page
	 	redirect('broadcastcalendar/index/','Refresh');
