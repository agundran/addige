...
class Booking_monthlydetailallmodel extends CI_Model{
	...
	$BMEndDate = date('Y-m-d',strtotime($EndDate)) or date('Y/m/d',strtotime($EndDate));
	  // $this->db->cache_on();
		$query = $this->db->query("SELECT 
								...
								WHERE 
								(c1.SiteName = "."'".$SiteName."'". ') and c1.billing_type = 0 '
								." and "."
...
function ComputeContractValue($ContractNo){
	...
	foreach ( $query->result() as $row)
			{
		       $cv =  $this->GetDistributionAmount($row->Line,$row->StartDate,$row->EndDate,$row->Distribution, $row->UnitPrice);
...
function GetDistributionAmount($line,$aStartDate, $aEndDate, $Dist , $Price){
