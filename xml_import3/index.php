...
if($Roger>0) {echo "File: " . dirname(__FILE__) . "Line: " . __LINE__ . ".\n";}
...
require_once( 'parse_xml.php - edit 1' );
...
GLOBAL $DEBUG;
	...
	if ($DEBUG) 
	{
		message_log_append( $msg_log, "state changed to $p_state\n" );
	}
} // next_state
...
//					$key . $key . '</label>' . "\n";
		...
		{
			$syscode_dropdown = " or select a SysCode: " . "<select name=\"syscode\">\n";
			foreach ($syscode_list as $key => $syscode) 
			{
				$selected = ($p_cont_key === $key) ? ' selected="selected"' : '';
				$syscode_dropdown .= "<option$selected value=$key>" . "$syscode</option>\n";
			}
			$syscode_dropdown .= "</select>&nbsp;";
			$syscode_dropdown .= "<input type=\"submit\" name=\"action\" " . "value=\"Select\">\n";
		} // if
    ...
    if (isset( $p_det[ 'MSG_LOG' ] )) 
	{
		...
		foreach ($p_det['MSG_LOG'] as $p_log) 
		{
			if ($p_log[2] == ERR_NO_NETWORK) 
			{
				...
				$button = '<a href="form_handler.php?' . 'action=DelNet' . '&' . 'LineID=' . $p_det['LineID'] . '" ' .
					'class="error" ' . 	'title="Delete Network ' . $p_det['Network'] . '" ' . '>&nbsp;Delete&nbsp;</a>';
				break;
			} // if
		} // foreach
		$ln .= "  <tr><td>$button</td>\n";
...
/////////////////////////////////
			case ($ps_state == 'CHECK_XML_FILENAME'):
				...
				$filnam = isset( $_FILES[ $input_nam ] ) ?  $_FILES[ $input_nam ][ 'name' ] : NULL;
// The full path to where the file was uploaded on the server
				$filtmp = isset( $_FILES[ $input_nam ] ) ?  $_FILES[ $input_nam ][ 'tmp_name' ] : NULL;
...
//echo "vac failed<br>";
				...
				next_state( ($ps_state == 'VERIFY_CAMPAIGN') ? 'DISPLAY_CAMPAIGN' : 'DISPLAY_CONTRACT' );
				break;
...
//echo "$ps_state<br>";
				...
				else 
				{
					...
					else 
					{
						...
						echo nav_buttons( array_keys( $headers ), TRUE, $headers, $cont_key );
...
// were deleted, and what their total value was.
				$spots = 0;
				$value = 0;
				$details[ $cont_key ] = delete_detail_lineid( $details[ $cont_key ], $lineID, $spots, $value );
...
// were deleted, and what their total value was.
				$spots = 0;
				$value = 0;
				$details[ $cont_key ] = delete_detail_network( $details[ $cont_key ], $network, $spots, $value );
// subtract deleted spots and value from header 'detail_*' totals
				$headers[ $cont_key ][ 'detail_spots' ] -= $spots;
				$v = $headers[ $cont_key ][ 'detail_cost' ];
				$v = bcsub( $v, $value, 2 );
